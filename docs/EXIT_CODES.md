...
- `init`, `list`, `status`, `summary`, `doctor` succeeded
- `run` completed with reviewer `reject` and retries remaining
- `run` completed with reviewer `stop` (state persisted for inspection)
- `run` / `auto` completed with tests green + review approve and handoff-ready branch (`success=ready_for_handoff`, default when `auto_merge=false`)
- `run` / `auto` completed with merge when `auto_merge=true` (`success=merged`)
...
Note: `run` returns 0 on reviewer `stop`, while `auto` classifies stop via heuristics — fixable stops enter repair; terminal stops exit 1 with structured `failure.report.json`. Prefer `status --json` / `summary --json` (`next_action`, `failure`, `success`) when polling detached runs.
...
| `auto` | done / handoff | config stop (incl. missing test_command) / max iter / merge notify | task failed / phase failure |
